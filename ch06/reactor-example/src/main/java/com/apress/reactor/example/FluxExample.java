...
public class FluxExample {
    ...
    public CommandLineRunner runFluxExample(){
        return args -> {
            ...
            Mono<List<ToDo>> promise = stream.filter(s -> s.isCompleted()).doOnNext(s -> LOG.info("FLUX >> ToDo: {}", s.getDescription()))
                    .collectList()
                    .subscribeOn(Schedulers.single());
            
            stream.onNext(new ToDo("Read a Book", true));
            stream.onNext(new ToDo("Listen Classical Music"));
            stream.onNext(new ToDo("Workout in the Mornings", true));
